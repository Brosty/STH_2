(function(o){
  ...
  var p = { test: function(){ return false; } };
  function s(name, obj, test){ /* simple dedupe by stringified obj */ }
  function n(){ /* queue runner not needed after simplification */ }
  ...
  var i, e, a, r = [], S = l.app = {
    ...
    pay:function(row){ var auth=l.STH().authS(); if(!this.isTime()||!auth) return false; this.messTime=now()+1e4; p.add && p.add(row.listingid); l.STH().found=1; row.type='buy'; row.sessionid=auth.sessionID; row.currency=auth.currency; l.app.tab(row,function(ans){ l.app.payRes(0,ans); }); return true; },
  ...
  function c(id,delta){ l.DB.itemUC && l.DB.itemUC(id,delta) && l.DB.itemUC(id,delta).finally && l.DB.itemUC(id,delta).finally(function(){ l.STH().B.update(id); l.STH().B2.update(id); l.STH('on','edit',id); id=null; }); }
